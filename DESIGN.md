...
Tool layout (`.main-container`): above 1024px, two columns — the form column (`1fr`) on the left and the preview column (`400px`) on the right, gap 32px, `align-items: start`. From 721px to 1024px the two columns stay but the preview narrows to `300px` (gap 24px) and the field grid drops to one column, so tablets never get a full-width preview card with a tiny code in it. The preview panel is sticky (`top: 80px` to clear the 56px navbar, `max-height: calc(100vh - 104px)`, `overflow-y: auto`). At 720px and below the grid collapses to one column and the **preview comes first** (`order: -1`) and is no longer sticky, so the result and the download buttons are visible before the form.

Every full-width band (navbar, hero, tool, steps strip, page header, content sections, footer) aligns to the same container: `max-width: 1200px; margin: 0 auto; padding: 0 24px` (16px below 768px). Bands carry only vertical padding; the horizontal inset always comes from the container so left edges line up across the page.
...
Field grid inside a card: short fields sit in a two-column grid (`grid-template-columns: 1fr 1fr`, column gap 16px, row gap 24px); a long field such as the URL spans both columns (`grid-column: 1 / -1`). At 1024px and below every field is full width (the form column is too narrow for two). Label to input gap 6px. Helper text is shown only where it prevents a mistake, never longer than one line; examples go into placeholders instead. Cards are separated by 24px.

Explanatory content on the Guide and FAQ pages sits in a centered reading column: every `.content-inner` on those pages carries `content-inner--narrow` (`max-width: 768px`, i.e. 720px of text plus the 24px insets), including the page header. Tables live inside that column and scroll horizontally when they need more room. Nothing hugs the left edge of the wide 1200px container. Vertical rhythm: 64px between sections on desktop, 48px below 768px; 16px between a table and the paragraph that follows it. Sections alternate flat backgrounds (white, then `--bg-light`) to give the page a rhythm without decoration.
...
Card per §5.1 with 24px padding. Contents, top to bottom: panel heading; the QR preview centered in a white box (`--radius`, 1px `--border-light`, 16px padding, canvas scaled to at most 280px wide so the download buttons stay above the fold); the URL preview (§5.9); a resolution `<select>` (§5.3) with a label and short option labels (`1200 px · print` style) so they fit a 300px column; a full-width primary button "Download PNG" and a full-width secondary button "Download SVG" stacked with 8px gap; a tertiary text-style "Regenerate" control (transparent background, `--primary-color` text, 44px tall). Before anything is generated the preview box uses `--bg-tertiary`.
